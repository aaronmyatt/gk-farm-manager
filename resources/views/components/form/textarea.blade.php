<div>
    <label for="{{ $label }}-{{ $attributes->wire('model')->value() }}"
        class="block text-sm font-medium text-gray-700">{{ ucfirst($label) }}</label>
    <div class="relative mt-1 rounded-md shadow-sm">
        <textarea 
            {{ $attributes->wire('model') }}
            name="{{$label}}-{{ $attributes->wire('model')->value() }}" id="{{ $label }}-{{ $attributes->wire('model')->value() }}"
            ...
            placeholder="{{ $placeholder }}" aria-invalid="true" aria-describedby="{{ $label }}-error"></textarea>
